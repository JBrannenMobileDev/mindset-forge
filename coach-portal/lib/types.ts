/**
 * Client-side mirror of `functions/src/coach_types.ts` in the main app repo.
 *
 * This repo is self-contained and must not import across the repo boundary,
 * so these types are copied by hand. Keep them in sync with the frozen
 * "Shared contracts" section of the coach team portal plan whenever the
 * server-side contract changes.
 *
 * Conventions, matching the server:
 ...
 * the portal.
 *
 * PRIVACY BOUNDARY: this document never carries raw journal text. No
 * `content` field, no verbatim quotes inside `summary` or `themes`.
 */
